import { Expression, NativeFunc } from "./ast.ts";
import { nativeFuncs } from "./core.ts";
import { Literal } from "./lexer.ts";
import { err } from "./utils.ts";

export type GosFunc = { params: string[]; body: Expression; type: string };
type FuncVal = GosFunc | NativeFunc;
...
export class Context {
  ...
  constructor() {
    this.vars[0].set("gos", Date.now());
    this.vars[0].set("NN", true);
